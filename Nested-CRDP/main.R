...
response_prob_a2_ab <- 0.9  # P( Response == 1 | a1 = a, a2 = b )
response_prob_a2_ba <- 0.5
response_prob_a2_bb <- 0.5

# Hypothesis testing parameters
null_hypothesis <- "a2.combined"  # c("a1", "a2.combined", "a2.given.a1", "a2.given.r")
...
for (sim in 1:N_SIM) {
  ...
  for (t in 1:N_PATIENTS) {
    n <- N_PATIENTS - t + 1
    optimal_action <- GetOptimalAction(optimal_policy$Q1$ACTION, n, i, j, k, backend_language)
  ...
  for (group in 1:4) {
    ...
    for (t in 1:group_size) {
      ...
      n <- group_size - t + 1
      optimal_action <- GetOptimalAction(optimal_group_policy, n, i, j, k, backend_language)
